...
struct
	...
	datatype frag = LITERAL of TigerTemp.label * string
								| FUNC of { body:stm list, frame: frame }
	    ...
	    in	case splitlast b of
		  ...
		  | (most,CJUMP(opr,x,y,t,f)) =>
		    (case (tabBusca(t,table), tabBusca(f,table)) of
		      ...
		      | _ =>	
		      	let	
		      		val f' = TigerTemp.newlabel()
						in 
							most @ [CJUMP(opr,x,y,t,f'), LABEL f', JUMP(NAME f,[f])] @ getnext(table,rest)
						end)
	...
	fun canonize [] = []
		| canonize (STRING x :: lfrag) = LITERAL x :: canonize lfrag
		| canonize (PROC {body, frame} :: lfrag) = FUNC {body=canon body, frame=frame} :: canonize lfrag
end
